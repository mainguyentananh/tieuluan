...
<div class="container space-s">
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
		<h1 class="h3 mb-0 text-gray-800">Tìm Kiếm: ${sach}</h1>
	</div>
	...
	<ul class="pagination  justify-content-center">
		...
		<c:if test="${param.page >= 10 && not empty param.page}">
			<li class="page-item"><a class="page-link"
				href="<c:url value="/search?page=1&sach=${sach}"/>">First</a></li>
		</c:if>
		<c:if test="${param.page > 1}">
			<li class="page-item"><a class="page-link"
				href="<c:url value="/search?page=${param.page-1}&sach=${sach}"/>">Previous</a></li>
		</c:if>
			
			<c:forEach begin="${start}" end="${end}" step="1" var="page">
					<c:if test="${disabled == page && start != 0}">
					<li class="page-item disabled"><a class="page-link"
						href="<c:url value="/search?page=${page}&sach=${sach}"/>">${page}</a></li>
					</c:if>
					<c:if test="${disabled != page && start != 0}">
					<li class="page-item"><a class="page-link"
						href="<c:url value="/search?page=${page}&sach=${sach}"/>">${page}</a></li>
		...
		<c:if test="${param.page < total || empty param.page}">
			<li class="page-item"><a class="page-link"
				href="<c:url value="/search?page=${param.page+1}&sach=${sach}"/>">Next</a></li>
			<li class="page-item"><a class="page-link"
				href="<c:url value="/search?page=${total}&sach=${sach}"/>">End</a></li>
		</c:if>
		</c:if>
	</ul>
</nav>
